				...
				if ( ! current_user_can( 'manage_options' ) ) {
					if ( $devOptions['bindhost'] != "" ) {
						if ( is_user_logged_in() ) {
							...
							if ( $bind ) {
                                //If query is already enclosed in parenthesis, only modify enough to add username to search for
                                if ( strpos( $ldapquery[0], "(" ) === 0 ) {
                                    $query = "(&" . $ldapquery[0] . "(" . $devOptions['loginfield'] . "=" . $wpUser->user_login . "))";
                                }
                                //Otherwise add parens and username
                                else {
                                    $query = "(&(" . $ldapquery[0] . ")(" . $devOptions['loginfield'] . "=" . $wpUser->user_login . "))";
                                }
								$sr = ldap_search( $ldap, $devOptions['searchbase'], $query, array( $devOptions['loginfield'] ), 0, 1 );
								if ( $sr === false ) {
...
<div id="lock-by-ldap-query_option_page" style="width:80%">
    <form method="post" action="<?php echo $_SERVER["REQUEST_URI"]; ?>">
        ...
        <h2>Lock By LDAP Query Settings</h2>
        <p>
            <label for='lockByLDAPQuery_bindhost' style='font-size: larger; font-weight: bold;'>LDAP Host</label>
            <br />
            <input id='lockByLDAPQuery_bindhost' type='text' name='lockByLDAPQuery_bindhost' value='<?php echo $devOptions['bindhost'];?>' style='width: 250px;'>
            <br />
            The name or IP address of the LDAP server. The protocol should be left out. (e.g., ldap.example.com)
        </p>
        <p>
            <label for='lockByLDAPQuery_searchbase' style='font-size: larger; font-weight: bold;'>Search Base</label>
            <br />
            <input id='lockByLDAPQuery_searchbase' type='text' name='lockByLDAPQuery_searchbase' value='<?php echo $devOptions['searchbase'];?>' style='width: 250px;'>
            <br />
            The base DN in which to carry out LDAP searches.
        </p>
        <p>
            <label for='lockByLDAPQuery_loginfield' style='font-size: larger; font-weight: bold;'>Login Attribute</label>
            <br />
            <input id='lockByLDAPQuery_loginfield' type='text' name='lockByLDAPQuery_loginfield' value='<?php echo $devOptions['loginfield'];?>' style='width: 250px;'>
            <br />
            The attribute in LDAP that needs to match the WP login name. (e.g., sAMAccountName if using AD)
        </p>
        <p>
            <label for='lockByLDAPQuery_binduser' style='font-size: larger; font-weight: bold;'>LDAP Bind User</label>
            <br />
            <input id='lockByLDAPQuery_binduser' type='text' name='lockByLDAPQuery_binduser' value='<?php echo $devOptions['binduser'];?>' style='width: 250px;'>
            <br />
            Leave blank for anonymous bind. Should be in DN format.
        </p>
        <p>
            <label for='lockByLDAPQuery_bindpassword' style='font-size: larger; font-weight: bold;'>LDAP Bind Password</label>
            <br />
            <input id='lockByLDAPQuery_bindpassword' type='password' name='lockByLDAPQuery_bindpassword' value='<?php echo $devOptions['bindpassword'];?>' style='width: 250px;'>
            <br />
            The password for the user above.
        </p>
        <p>
            <label for='lockByLDAPQuery_usessl' style='font-size: larger; font-weight: bold;'>Use SSL</label>
            <br />
            <input id='lockByLDAPQuery_usessl' type='checkbox' name='lockByLDAPQuery_usessl' value='true'<?php echo ( $devOptions['usessl'] ? " checked" : "" );?>> Enabled
            <br />
            Default is disabled (no SSL).
        </p>
        <p>
            <label for='lockByLDAPQuery_referrals' style='font-size: larger; font-weight: bold;'>Use LDAP_OPT_REFERRALS = 1</label>
            <br />
            <input id='lockByLDAPQuery_referrals' type='checkbox' name='lockByLDAPQuery_referrals' value='true'<?php echo ( $devOptions['referrals'] ? " checked" : "" );?>> Enabled
            <br />
            Setting to disabled is often necessary when using Active Directory.
        </p>
        <p>
            <label for='lockByLDAPQuery_protocolversion3' style='font-size: larger; font-weight: bold;'>Use LDAP V3</label>
            <br />
            <input id='lockByLDAPQuery_protocolversion3' type='checkbox' name='lockByLDAPQuery_protocolversion3' value='true'<?php echo ( $devOptions['protocolversion3'] ? " checked" : "" );?>> Enabled
            <br />
            Default is enabled.
        </p>
